  ...
  return {
    month_list = function (arg, has_block)
      if arg and arg.include_tags then
        local sections = models.section:find_all("tag like ?", { arg.include_tags })
        local section_ids = {}
        ...
        if not has_block then
          out = {}
          template = load_template(arg.template or "month_list.html")
        end
        for _, month in ipairs(months) do
          ...
          env.uri = web:link("/archive/" .. env.year .. "/" .. env.month_padded)
          if has_block then
            cosmo.yield(env)
          else
...
function plugins.section_list(web)
  return {
    section_list = function (arg, has_block)
      ...
      if not has_block then
        out = {}
        template = load_template(template_name or "section_list.html")
      end
...
local function get_posts(web, condition, args, count, template)
  local posts = models.post:find_all(condition, args)
  local cur_date, out
...
function plugins.home(web)
  return {
    headlines = function (arg, has_block)
      ...
      return get_posts(web, "in_home = ? and published = ?", { order = "published_at desc", true, true }, nil, template)
    end
  }
end
  ...
  return {
    show_posts = function (arg, has_block)
      ...
      if arg and arg.archive and web.input.month and web.input.year then
        ...
        date_end = os.time({
          year = web.input.year + math.floor(web.input.month / 12),
      ...
      return get_posts(web, 
        "published = ? AND section_id = ? AND published_at >= ? AND published_at <= ?",
        {
